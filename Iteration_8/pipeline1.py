...
UNSUPERVISED METHODS (PCA, clustering, dimensionality reduction) — KEEP LABELS OUT OF X
- This applies ONLY to unsupervised techniques like PCA/K-Means/hierarchical clustering,
  NOT to classification or regression, where a categorical predictor is a legitimate
  encoded input feature — do not apply this rule there.
- If a column is a known ground-truth class/category/species/label column (a column
  that will later be used to color a plot, build a contingency table, or compute
  ARI/NMI/accuracy against the unsupervised result), it must be EXCLUDED from the
  feature matrix that PCA/clustering is actually fit on. Do not one-hot encode it
  and fold it back in as an input feature (e.g. pd.get_dummies on a Species column
  before PCA) — this leaks the label into the very structure you are trying to
  discover unsupervised, and inflates/distorts explained variance, cluster
  separation, and any subsequent comparison against that same label as if it were
  an independent validation.
- Concretely: build X for PCA/clustering only from the genuine numeric measurement
  columns (after dropping ID/index columns per the usual rule), and keep the
  label/category column completely separate, used only afterward for
  visualization or for comparison against the unsupervised result — never as
  one of the columns being reduced or clustered.
- Print explicitly which columns went into X for the unsupervised step, so this is
  auditable (e.g. "Features used for PCA: SepalLengthCm, SepalWidthCm, ...
  (Species excluded — reserved for post-hoc comparison)").
...
- Save with EXACTLY this call, using this EXACT path and filename, character for
  character — do NOT rename it to anything more "descriptive" like
  pca_visualization.png, clusters.png, stress_strain.png, etc. The pipeline's
  post-run check looks for this exact path and will report a false failure if
  the filename is changed even slightly:
      plt.savefig("./results/output_plot.png", dpi=150, bbox_inches="tight")
- Never call plt.show()

NEVER HARDCODE ASSUMED CATEGORY/LABEL STRING VALUES
- When coloring, filtering, or mapping by a categorical column (species, class,
  category, label, etc.), NEVER hardcode the exact string values you assume are
  in that column (e.g. assuming a Species column contains 'setosa' when the real
  data contains 'Iris-setosa'). A mismatched hardcoded string silently matches
  nothing, producing a plot where every point gets the same fallback color/value
  with no error or warning raised — this has actually happened.
- Always read the real category values directly from the data first:
      categories = sorted(df[label_col].unique())
      print(f"Categories found in '{label_col}': {categories}")
  Then build any color mapping, palette, or dummy-variable structure FROM this
  list — e.g. `{cat: i for i, cat in enumerate(categories)}` — rather than typing
  out label strings from assumption or memory.
- Apply the same rule to any other string-matching logic on data values (unit
  suffixes, boolean-like text such as "yes"/"no" vs "Y"/"N", etc.): derive the
  actual distinct values present with .unique() before writing matching logic
  against them.

FORCE AXIS LIMITS TO THE ACTUAL DATA RANGE — CRITICAL, ALWAYS DO THIS
- Any construction/reference line — a 0.2% offset line, an extrapolated trend line, a
  confidence line, an asymptote — must NEVER be plotted across the full x array by
  default. Lines like `offset_line = slope * (x - 0.002) + intercept` are, by
  construction, guaranteed to run far above/below the real data at the edges of the
  x range (this has actually happened: an offset line with a steel-like slope reached
  ~57,000 on the y-axis while the real data topped out under 700, crushing the entire
  real curve into an invisible sliver at the bottom of the plot).
- To prevent this, ALWAYS explicitly set the axis limits from the real data at the end
  of the plotting code, after all lines/scatter are added, using a small margin:
      y_margin = 0.08 * (np.max(y) - np.min(y)) if np.max(y) > np.min(y) else 1.0
      plt.ylim(np.min(y) - y_margin, np.max(y) + y_margin)
      x_margin = 0.05 * (np.max(x) - np.min(x)) if np.max(x) > np.min(x) else 1.0
      plt.xlim(np.min(x) - x_margin, np.max(x) + x_margin)
  Set these limits AFTER every plt.plot/scatter/fill_between call, right before
  plt.savefig, so nothing plotted afterward can silently widen the axes again.
- Additionally, any construction line that is only meant to locate an intersection
  point (like a 0.2% offset line) should itself only be drawn across a short, relevant
  x-range — e.g. from the start of the elastic region out to a bit past where it
  crosses the curve — not across the entire x array:
      offset_x_max = min(np.max(x), (yield_strain if yield_strain is not None
                                      else np.max(x_elastic)) * 1.5)
      offset_x_plot = np.linspace(0, offset_x_max, 100)
      offset_y_plot = coeffs_elastic[0] * (offset_x_plot - 0.002) + coeffs_elastic[1]
      plt.plot(offset_x_plot, offset_y_plot, "g--", label="0.2% Offset Line")
  Never reuse the full `x` array to draw a steep construction line end-to-end.
...
POLYNOMIAL DEGREE SELECTION — USE CROSS-VALIDATION, NOT IN-SAMPLE AIC
- Do NOT select polynomial degree using in-sample AIC computed on the full dataset.
  In-sample AIC (and in-sample RSS/R²) on small, noisy real-world datasets almost
  always keeps improving as degree increases, because a higher-degree polynomial can
  always bend to chase noise in the training points themselves — it has no way to
  detect that it is overfitting. This is exactly why higher degrees kept winning and
  producing wiggly, physically implausible fits.
- Instead, select degree using leave-one-out cross-validated error. Copy this pattern:
      candidate_degrees = [1, 2, 3, 4]   # hard cap at 4, never scale with n
      cv_errors = []
      for degree in candidate_degrees:
          if len(x) <= degree + 1:
              cv_errors.append(np.inf)   # not enough points to fit + validate
              continue
          sq_errors = []
          for i in range(len(x)):
              x_train = np.delete(x, i)
              y_train = np.delete(y_fit, i)
              c = np.polyfit(x_train, y_train, degree)
              pred = np.polyval(c, x[i])
              sq_errors.append((y_fit[i] - pred) ** 2)
          cv_mse = float(np.mean(sq_errors))
          cv_errors.append(cv_mse)
          print(f"  degree={degree}  CV_MSE={cv_mse:.4f}")
      best_degree = candidate_degrees[int(np.argmin(cv_errors))]
      coeffs = np.polyfit(x, y_fit, best_degree)
      print(f"Selected degree {best_degree} by lowest leave-one-out CV error "
            f"(not in-sample AIC, to avoid rewarding overfitting).")
- This naturally penalizes high-degree fits: a wiggly degree-4 curve that chases noise
  will fit each left-out point poorly, so its CV_MSE will be visibly worse than a
  simpler degree's, even though its in-sample AIC/R² would look better.
- After selecting the best degree by CV error, compute and print R² in the ORIGINAL
  (linear) units of y regardless of which space was fit in, so fit quality is always
  interpretable
...
- SANITY CHECK ON CURVE SHAPE: after fitting, you MUST copy the following code
  EXACTLY as written, with no modification, no normalization, and no division of
  the sign-change counts. Do not "simplify" or rewrite this block — copy it
  verbatim, substituting only your actual variable names for x, y, coeffs,
  and best_degree if they differ:

  IMPORTANT — SCOPE: this entire check is gated on `is_data_monotonic`, which is
  computed FROM THE ACTUAL DATA, never assumed. If the real data genuinely rises
  and falls (e.g. a sinusoidal signal, a seasonal trend, a peak-shaped response),
  `is_data_monotonic` will correctly be False and this whole block is a no-op —
  it will NEVER force a straight-line or low-degree fit onto legitimately
  non-monotonic data. It only intervenes when the data itself is monotonic (or
  flat/plateauing) but the fitted polynomial wiggles beyond what the data shows.

      # --- oscillation sanity check (copy exactly, do not modify) ---
      data_signs = np.sign(np.diff(y))
      data_signs = data_signs[data_signs != 0]
      data_sign_changes = int(np.sum(np.diff(data_signs) != 0))
      is_data_monotonic = data_sign_changes <= 1

      fit_signs = np.sign(np.diff(np.polyval(coeffs, x)))
      fit_signs = fit_signs[fit_signs != 0]
      fit_sign_changes = int(np.sum(np.diff(fit_signs) != 0))

      # Amplitude check: even a single extra "turning point" is only a real
      # problem if it produces a visible dip/bump, not a sub-noise wiggle.
      y_range = float(np.max(y) - np.min(y)) if len(y) > 1 else 0.0
      fit_vals = np.polyval(coeffs, x)
      dip_amplitude = float(np.max(fit_vals) - np.min(fit_vals[np.argmax(fit_vals):])) \
          if np.argmax(fit_vals) < len(fit_vals) - 1 else 0.0
      is_visible_dip = y_range > 0 and (dip_amplitude / y_range) > 0.02

      if is_data_monotonic and fit_sign_changes > 1 and is_visible_dip:
          print(f"WARNING: Degree {best_degree} fit was non-monotonic "
                f"({fit_sign_changes} sign changes, dip={100*dip_amplitude/y_range:.1f}% "
                f"of data range) despite monotonic data — falling back to a lower degree.")
          for fallback_degree in range(best_degree - 1, 0, -1):
              coeffs_fb    = np.polyfit(x_fit, y_fit, fallback_degree)
              fit_signs_fb = np.sign(np.diff(np.polyval(coeffs_fb, x)))
              fit_signs_fb = fit_signs_fb[fit_signs_fb != 0]
              fit_vals_fb  = np.polyval(coeffs_fb, x)
              dip_fb = float(np.max(fit_vals_fb) - np.min(fit_vals_fb[np.argmax(fit_vals_fb):])) \
                  if np.argmax(fit_vals_fb) < len(fit_vals_fb) - 1 else 0.0
              ok_signs = int(np.sum(np.diff(fit_signs_fb) != 0)) <= 1
              ok_dip   = y_range == 0 or (dip_fb / y_range) <= 0.02
              if ok_signs and ok_dip:
                  coeffs = coeffs_fb
                  best_degree = fallback_degree
                  print(f"Successfully fell back to degree {fallback_degree}.")
                  break
      # --- end oscillation sanity check ---

  CRITICAL: `data_sign_changes` and `fit_sign_changes` above are RAW INTEGER
  COUNTS of sign changes, never divide them by array length or any other
  value to turn them into a ratio/proportion — a proportion is always <= 1
  and can never trigger the ">1" or "<=1" comparisons above, silently
  disabling this entire safety check. This exact bug has happened before:
  do not compute anything like `count / (len(diff) - 1)`.
...
def pipeline(data_path, user_request, max_fixes=MAX_FIXES):
    ...
    # Analysis-only rules for pass 1: strip PLOT_RULES out even when do_plot is
    # True, and instead explicitly forbid matplotlib in this pass. The plot
    # (if any) is generated separately in pass 2 below. This keeps each LLM
    # call well under the token budget so a long fitting/bootstrap/classification
    # script can't truncate mid-generation and silently drop the plotting code
    # (previously observed as done=length with output_plot.png never created).
    analysis_rules = BASE_RULES
    analysis_rules += "\n\n" + NO_PLOT_RULES
    if do_fitting:
        analysis_rules += "\n\n" + FITTING_RULES
    if do_classification:
    ...
    pass1_note = (
        "This is PASS 1 of 2. Write ONLY the data loading, preprocessing, "
        "computation, fitting, and printed-output code. Do NOT import matplotlib "
        "and do NOT produce any plot in this pass — plotting is handled in a "
        "separate pass afterward using the variables you define here. Make sure "
        "every variable the plot will need (e.g. x, y, x_range, mean/lower/upper "
        "bands, coeffs, best_degree, cluster labels, etc.) is clearly defined by "
        "name and left in scope at the end of the script."
    )
    ...
    # Generate — pass 1 (analysis only)
    print("\n=== GENERATING (pass 1: analysis) ===")
    ...
    # Generate — pass 2 (plotting only), appended onto pass 1's code
    if do_plot:
        print("\n=== GENERATING (pass 2: plotting) ===")
        plot_prompt = (
            f"Below is a complete, already-working Python script that loads the "
            f"data and performs all the required computation/fitting/analysis. "
            f"Do NOT repeat, rewrite, or duplicate any of this code.\n\n"
            f"--- EXISTING SCRIPT (for context only) ---\n{code}\n"
            f"--- END EXISTING SCRIPT ---\n\n"
            f"{PLOT_RULES}\n\n"
            f"Write ONLY the additional Python code to append to the end of the "
            f"script above that produces the required plot, using the variables "
            f"already defined above (do not redefine them). Include the "
            f"matplotlib backend setup and imports (matplotlib.use(\"Agg\") and "
            f"import matplotlib.pyplot as plt) at the top of what you return, "
            f"since this snippet will be appended after the existing imports. "
            f"Return ONLY the raw Python code to append — no markdown, no "
            f"explanation, and do not repeat any of the existing script."
        )
        plot_history = [{"role": "user", "content": plot_prompt}]
        raw_plot = call_ollama(plot_history, SYSTEM_GENERATOR)
        plot_code = strip_special_tokens(strip_fences(raw_plot))
        print(f"  Generated {len(plot_code.splitlines())} lines.")

        if len(plot_code.splitlines()) >= 3:
            code = code.rstrip() + "\n\n# --- Plotting (pass 2) ---\n" + plot_code
        else:
            print("  ⚠ Plot pass returned too little — continuing without a plot.")
    ...
    if ok:
        ...
        if os.path.exists(PLOT_PATH):
            ...
            print("  ⚠ WARNING: a plot was required for this request but "
                  f"{PLOT_PATH} was not produced. The script ran without error "
                  "but never reached/executed the plotting code — inspect "
                  f"{SCRIPT_PATH} for a silent failure in the appended plot pass.")
    else:
